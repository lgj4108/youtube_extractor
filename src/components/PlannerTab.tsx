...
export default function PlannerTab() {
    ...
    const handleGenerateAiPlans = async () => {
        ...
        } catch (error: unknown) { setAiError(getErrorMessage(error, '기획안 생성 중 오류가 발생했습니다.')); } finally { setIsGenerating(false); }
    };
    ...
    const currentDisplayData = effectiveViewMode === 'search' ? videos : savedVideos;
    ...
    return (
        <div className="animate-fadeIn relative">
            ...
            {(videos.length > 0 || savedVideos.length > 0) && !loading && (
                <>
                    ...
                    <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4 border-b border-slate-200 dark:border-slate-700 pb-4">
                        <div className="flex bg-slate-200/50 dark:bg-slate-800 p-1 rounded-lg">
                            <button onClick={() => setViewMode('search')} className={`px-4 py-1.5 text-xs font-bold rounded-md transition-colors ${effectiveViewMode === 'search' ? 'bg-white shadow-sm text-indigo-600' : 'text-slate-500'}`}>검색 결과 ({videos.length})</button>
                            <button onClick={() => setViewMode('saved')} className={`px-4 py-1.5 text-xs font-bold rounded-md transition-colors ${effectiveViewMode === 'saved' ? 'bg-white shadow-sm text-indigo-600' : 'text-slate-500'}`}>⭐️ 스크랩 ({savedVideos.length})</button>
                        </div>
                    ...
                    <div className="grid grid-cols-1 gap-4">
                        {currentDisplayData.length > 0
                            ? currentDisplayData.map((video, index) => (
                                <VideoCard key={video.videoId} video={video} index={effectiveViewMode === 'search' ? index : undefined} isSaved={savedVideos.some(v => v.videoId === video.videoId)} onToggleSave={handleToggleSave} />
                            ))
                            : <div className="rounded-xl border border-dashed border-slate-300 p-8 text-center text-sm text-slate-500">이 목록에는 아직 영상이 없습니다.</div>}
                    </div>
                </>
            )}
                ...
                <span className={`inline-block px-2 py-1 text-[10px] font-bold rounded-md mb-2 ${viewingScript.format === 'long' ? 'bg-indigo-100 text-indigo-700' : 'bg-slate-100 text-slate-700'}`}>
                  {viewingScript.format === 'long' ? '📈 롱폼 기획 스크립트' : '🎬 숏츠 대본'}
                        ...
                        <div className="p-4 border-t border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50 rounded-b-2xl flex justify-end">
                            <button
                                onClick={() => copyText(viewingScript.script).catch((error: unknown) => setAiError(getErrorMessage(error)))}
